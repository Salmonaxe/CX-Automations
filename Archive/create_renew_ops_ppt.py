...
def create_ppt_for_filter(filtered, fy_start, fy_end, excel_filename, suffix, customer_name, initial_fy, final_fy, file_creation_date, title_prefix, columns, min_atr, total_atr_by_deal):
    ...
    summary_slide = add_summary_table_slide(prs, aggregated_by_deal, fy_start, fy_end, value_col, title_prefix)
    ...
    for col in ['Product Amount (TCV) ($000s)', 'Service Amount (TCV) ($000s)', 'Expected ATR ($000s)', 'Prior ATR ($000s)']:
        if col in df.columns:
            df[col] = pd.to_numeric(
                df[col].astype(str).str.replace('[\$,]', '', regex=True).replace('nan', '0'),
                errors='coerce'
            ).fillna(0)
