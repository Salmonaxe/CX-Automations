...
def add_summary_table_slide(
    prs, filtered, fy_start, fy_end, value_col, opportunity_status
):
    """
    Adds a summary table slide to the PowerPoint presentation, filtered by opportunity status.

    Parameters:
        prs: pptx.Presentation object.
        filtered: pd.DataFrame, must have 'Account Name', 'Opportunity status', 'Close Date', and value_col.
        fy_start: str or datetime, start of fiscal year.
        fy_end: str or datetime, end of fiscal year.
        value_col: str, column to aggregate.
        opportunity_status: str, one of 'Lost', 'Booked', 'Active'.
    ...
    tf.text = f"{title_prefix} Summary Table - Expected TCV Aggregated by Month ($000s)"
...
def add_table_slides(prs, filtered, columns, title_prefix, account_col='Account Name'):
    for account in filtered[account_col].unique():
        ...
        for slide_idx, start_row in enumerate(range(0, num_rows, MAX_TABLE_ROWS_PER_SLIDE)):
            ...
            textbox = slide.shapes.add_textbox(left, top, width, height)
            tf = textbox.text_frame
            tf.text = f"{account} {title_prefix} " + (f"(page {slide_idx+1})" if num_slides > 1 else "")
...
def add_timeline_slides(prs, filtered, fy_start, fy_end, title_prefix, account_col='Account Name', min_tcv=0):
    for account in filtered[account_col].unique():
        ...
        for slide_idx, start_timeline in enumerate(range(0, n_timelines, MAX_TIMELINES_PER_SLIDE)):
            ...
            tf = textbox.text_frame
            tf.text = f"{account} {title_prefix} Timeline" + (f" (page {slide_idx+1})" if n_timelines > MAX_TIMELINES_PER_SLIDE else "")
            tf.paragraphs[0].font.size = Pt(16)
            tf.paragraphs[0].alignment = 1
...
def add_monthly_timeline_slides(prs, filtered, fy_start, fy_end):
    ...
    for month_start in months:
        ...
        slide.shapes.add_picture(img_stream, Inches(0.5), Inches(1.0), width=Inches(8.5))
...
def create_ppt_for_filter(prs, filtered, fy_start, fy_end, customer_name, initial_fy, final_fy, file_creation_date, title_prefix, columns, min_tcv=0):
    # --- TITLE SLIDE ---
    title_slide = prs.slides.add_slide(prs.slide_layouts[0])
    title = title_slide.shapes.title
    subtitle = title_slide.placeholders[1]
    title.text = f"{title_prefix} in {customer_name}"
    subtitle_text = (
    ...
    # --- SUMMARY TABLE SLIDE (add, then move to be slide 2) ---
    value_col = 'Expected Amount TCV ($000s)'
    summary_slide = add_summary_table_slide(prs, filtered, fy_start, fy_end, value_col, 'Booked')
    summary_slide = add_summary_table_slide(prs, filtered, fy_start, fy_end, value_col, 'Lost')
    summary_slide = add_summary_table_slide(prs, filtered, fy_start, fy_end, value_col, 'Active')
    # Move summary slide to position 1 (after title)
    xml_slides = prs.slides._sldIdLst
    slides = list(xml_slides)
    xml_slides.insert(1, slides[-1])  # Move last slide (summary) after first slide (title)
    del slides[-1]
...
def create_new_ops_ppt(initial_fy, final_fy, excel_filename, min_tcv=0):
    ...
    # --- NEW OPS PPT ---
    prs = Presentation()  # Initialize the presentation once
...
if __name__ == '__main__':
    ...
    try:
        create_new_ops_ppt(args.initial_fy, args.final_fy, args.excel_filename, args.min_tcv)
    except Exception as e:
